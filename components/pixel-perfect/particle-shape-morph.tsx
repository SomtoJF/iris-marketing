...
export type ParticleShapeName =
  "sphere" | "torus" | "helix" | "knot" | "cube" | "wave";
...
export interface ParticleShapeMorphProps {
  ...
  duration?: number;
  /** Automatically advance to the next shape. */
  auto?: boolean;
  /** Milliseconds between automatic morphs. */
  interval?: number;
  showLabels?: boolean;
}
...
function buildHelix(out: Float32Array, n: number) {
  ...
  for (let i = 0; i < strand; i++)
    place(strand + i, i / (strand - 1 || 1), 0, 1);
...
function buildKnot(out: Float32Array, n: number) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (a !== station) {
      ...
      centre(t + 1e-3, next);
      let len =
        Math.hypot(next[0] - here[0], next[1] - here[1], next[2] - here[2]) ||
        1;
    ...
    for (let k = 0; k < 3; k++)
      out[i * 3 + k] = here[k] + nrm[k] * cs + bin[k] * sn;
  }
}
...
const ParticleShapeMorph = ({
  ...
  useGSAP(
    () => {
      ...
      const update = () => {
        ...
        if (visible && !document.hidden && !reduced.matches) {
          ...
          frame = requestAnimationFrame(tick);
          if (auto && forms.length > 1) {
            autoTimer = setInterval(advance, Math.max(1000, interval));
          }
        }
      };
      ...
      const advance = () => {
        ...
        for (let i = 0; i < n; i++) {
          ...
          const my =
            (from[i * 3 + 1] + to[i * 3 + 1]) / 2 + jitter[i * 3 + 1] * 0.16;
          const mz =
            (from[i * 3 + 2] + to[i * 3 + 2]) / 2 + jitter[i * 3 + 2] * 0.24;
    ...
    { dependencies: [key, count, duration, auto, interval], scope: hostRef },
  );
